...
- Since v4.7.0, each upstream may have a static `available_models` list captured manually from its `/models` endpoint. Runtime requests never probe `/models`: an exact client-model match is forwarded, otherwise that upstream's `model`/`messages_model` default is used. Channels whose model probe fails can omit the list and retain default-only behavior.
